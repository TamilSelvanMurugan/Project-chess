...
function moment(position, dir, moves) {
    ...
    var y = position[1];
    var posi = [x, y];
    console.log("Starting Queen position x,y  " + posi)
    if (x >= 8 || y >= 8 || x < 0 || y < 0) {
        console.log("Check Your Poistion ")
    } else {
        switch (dir) {
            case "S":
                y = y + moves;
        ...
        if (((x >= 8) || (x < 0)) || ((y < 0) || (y >= 8))) {
            console.log("Oops! Out of the chess board !")
        } else {
            return arr;
        }
    }
...
function updatePosition(arr) {
    ...
    return (positionLog[arr[0]][arr[1]]);
}
...
for (i = 0; i < queens.length; i++) {
    ...
    if (arr != null) {
        ...
        if (final == true) {
            console.log("Already One Queen in the same position");
        } else {
            console.log("Final Position:" + updatePosition(arr));
